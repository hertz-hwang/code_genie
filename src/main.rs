...
mod config {
    // [文件路径]
    pub const FILE_FIXED: &str = "input-fixed.txt";           // 格式: 字根 [tab] 键位(单个或空格分隔多个)
    pub const FILE_DYNAMIC: &str = "input-roots.txt";         // 格式: 字根 (每行一个)
    pub const FILE_SPLITS: &str = "input-division.txt";       // 格式: 汉字 [tab] 字根1字根2... [tab] 字频
    ...
    pub const ALLOWED_KEYS: &str = "qwertyuiopasdfghjklzxcvbnm";

    // [目标函数权重]
    pub const WEIGHT_COLLISION_COUNT: f64 = 0.6;
    pub const WEIGHT_COLLISION_RATE: f64 = 0.8;
    pub const WEIGHT_EQUIVALENCE: f64 = 5.0;
    pub const WEIGHT_DISTRIBUTION: f64 = 0.5;
    pub const WEIGHT_EQUIV_CV: f64 = 0.45;
    ...
    pub const REHEAT_FACTOR: f64 = 2.0;
    pub const ACCEPTANCE_TARGET: f64 = 0.4;
}
...
impl OptContext {
    fn new(
        ...
        constrained_roots: &HashMap<String, Vec<u8>>,
        equiv_table: [[f64; EQUIV_TABLE_SIZE]; EQUIV_TABLE_SIZE],
        key_dist_config: [KeyDistConfig; EQUIV_TABLE_SIZE],
    ) -> Self {
        // 解析全局允许键位
        ...
        // 构建每个动态字根的允许键位列表
        let mut root_allowed_keys: Vec<Vec<u8>> = Vec::with_capacity(num_dynamic_roots);
        for root_str in dynamic_roots.iter() {
            if let Some(allowed) = constrained_roots.get(root_str) {
                root_allowed_keys.push(allowed.clone());
            } else {
                root_allowed_keys.push(global_allowed.clone());
            }
        }
        ...
        for (char_idx, (_, roots, freq)) in splits.iter().enumerate() {
            ...
            let mut used_dynamic_indices = HashSet::new();
            ...
            for root in roots.iter().take(MAX_PARTS) {
                ...
                if let Some(&key) = fixed_map.get(root) {
                    ...
                    info.parts[i] = (dyn_idx + 1000) as u16;
                    info.num_parts += 1;
...
impl Evaluator {
    ...
    fn get_score(&self, ctx: &OptContext) -> f64 {
        ...
        config::WEIGHT_COLLISION_COUNT * self.total_collisions as f64
            ...
            + config::WEIGHT_EQUIVALENCE * weighted_equiv * 1000.0
...
fn smart_init(ctx: &OptContext) -> Vec<u8> {
    ...
    let mut root_freq: Vec<(usize, usize)> = ctx
        .root_to_char_indices
        .iter()
        .enumerate()
        .map(|(i, v)| (i, v.len()))
    ...
    for (root_idx, _) in root_freq {
        let allowed = &ctx.root_allowed_keys[root_idx];
        
        let min_count = allowed.iter()
            .map(|&k| key_counts[k as usize])
            .min()
            .unwrap_or(0);
        
        let candidates: Vec<u8> = allowed.iter()
            .filter(|&&k| key_counts[k as usize] == min_count)
            .copied()
            .collect();
...
fn simulated_annealing(ctx: &OptContext, thread_id: usize) -> (Vec<u8>, f64, Metrics) {
    ...
    if rng.gen_bool(0.5) {
        for (i, val) in assignment.iter_mut().enumerate() {
            if rng.gen_bool(0.1) {
                let allowed = &ctx.root_allowed_keys[i];
    ...
    for step in 0..steps {
        ...
        if rng.gen::<f64>() < config::SWAP_PROBABILITY {
            ...
            if r1 != r2 {
                let k1 = assignment[r1];
                let k2 = assignment[r2];
                let can_swap = ctx.root_allowed_keys[r1].contains(&k2) 
                            && ctx.root_allowed_keys[r2].contains(&k1);
                if can_swap {
            ...
            let allowed = &ctx.root_allowed_keys[r];
        ...
        if current_score < best_score {
            ...
            if thread_id == 0 && best_score <= last_best_score - 0.1 {
                let m = best_metrics;
                println!(
                    "   [T0] 步数 {}/{} | 温度 {:.6} | (重码:{}, 重码率:{:.4}%, 当量:{:.4}, CV:{:.4}, 分布偏差:{:.4}) | 当前得分: {:.4}",
        ...
        if thread_id == 0 && step % report_interval == 0 && step > 0 {
            ...
            let m = evaluator.get_metrics(ctx);
            println!(
                "   [T0] 进度: {}% | 温度: {:.6} | (重码={}, 重码率={:.4}%, 当量={:.4}, CV={:.4}, 分布={:.4}) | 当前得分: {:.4} 🏆最优: {:.4}",
...
/// 格式支持:
///   - 单键位: "传	a" (真正固定，不参与优化)
///   - 多键位: "传	a d h f n" (受限动态，可在指定键位间移动)
fn load_fixed(path: &str) -> (HashMap<String, u8>, HashMap<String, Vec<u8>>) {
    let content = fs::read_to_string(path).expect("无法读取固定字根文件");
    let mut truly_fixed = HashMap::new();
...
/// 加载动态字根，合并受限动态字根
fn load_dynamic(path: &str, constrained_roots: &HashMap<String, Vec<u8>>) -> Vec<String> {
    let mut roots: Vec<String> = fs::read_to_string(path)
        .expect("无法读取动态字根文件")
        .lines()
    ...
    for root in constrained_roots.keys() {
...
fn check_and_report_validation(
    ...
    let (is_valid, missing_roots, missing_examples) = validate_roots(
        splits, fixed_map, constrained_roots, dynamic_roots
    );
...
fn main() {
    ...
    // 1. 加载数据
    let (fixed_map, constrained_roots) = load_fixed(config::FILE_FIXED);
    let dynamic_roots = load_dynamic(config::FILE_DYNAMIC, &constrained_roots);
    ...
    println!("\n数据加载完毕:");
    println!("  - 固定字根(单键): {}", fixed_map.len());
    println!("  - 受限字根(多键): {}", constrained_roots.len());
    println!("  - 动态字根总数: {}", dynamic_roots.len());
    println!("  - 汉字数量: {}", splits.len());
    println!("  - 总字频: {}", total_freq);

    // 2. 校验字根定义
    if !check_and_report_validation(&splits, &fixed_map, &constrained_roots, &dynamic_roots) {
        std::process::exit(1);
    }
...
fn save_results(
    ...
    root_out.push_str("# 动态字根键位分配:\n");

    for (i, root_str) in ctx.raw_dynamic_roots.iter().enumerate() {
        let key = key_to_char(assignment[i]);
...
fn save_equiv_distribution(ctx: &OptContext, assignment: &[u8]) {
    ...
    dist_out.push_str("#\n");
    dist_out.push_str("# 当量最高的20个高频字 (字频>500000):\n");
    dist_out.push_str("# 汉字\t当量\t字频\n");
    
    let mut count = 0;
    for (ch, equiv, freq) in &char_equivs {
        if *freq > 500000 && count < 20 {
    ...
    dist_out.push_str("#\n");
    dist_out.push_str("# 当量最低的20个高频字 (字频>500000):\n");
    
    let high_freq_chars: Vec<_> = char_equivs.iter().filter(|(_, _, f)| *f > 500000).collect();
